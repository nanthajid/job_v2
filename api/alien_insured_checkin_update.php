...
$pdo   = getDB();
$check = $pdo->prepare('SELECT 1 FROM ' . ALIEN_CHECKIN_DOC_TABLE . ' WHERE DocID = :id');
...
try {
    ...
    $upd = $pdo->prepare('UPDATE ' . ALIEN_CHECKIN_DOC_TABLE . ' SET
    ...
    $pdo->prepare('DELETE FROM ' . ALIEN_CHECKIN_PERSON_TABLE . ' WHERE DocID = :id')->execute(['id' => $docId]);
    alienInsuredInsertPersons($pdo, $docId, $persons, ALIEN_CHECKIN_PERSON_TABLE);
    $pdo->commit();
