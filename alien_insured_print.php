...
$stmt = $pdo->prepare('SELECT * FROM alien_insured_doc WHERE DocID = :id');
...
$pStmt = $pdo->prepare('SELECT * FROM alien_insured_person WHERE DocID = :id ORDER BY SeqNo ASC, PersonID ASC');
...
<head>
  ...
  <title>พิมพ์แบบขึ้นทะเบียนผู้ประกันตนแรงงานต่างด้าว | <?= htmlspecialchars($officeName) ?></title>
...
<div class="sheet">
  ...
  <div class="no-print">
    ...
    <a href="alien_insured.php" class="btn btn-outline-secondary btn-sm px-3" style="border-radius:8px;">ย้อนกลับ</a>
  </div>
